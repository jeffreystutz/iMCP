...
| [0010](0010-global-automatic-send-authorization-policy.md) | Accepted | Persist one global, binary Sending mode (Ask Before Sending / Send Automatically), app-owned and unreachable by any MCP caller, wired into both payload types of the unified `messages_send` tool. |
| [0011](0011-unified-messages-send-tool.md) | Accepted | Consolidate `messages_send` and `messages_send_attachment` into one public `messages_send` tool with mutually exclusive `body`/`attachment` payloads, enforced in code; internal pipelines unchanged. |
