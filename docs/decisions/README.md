...
| [0010](0010-global-automatic-send-authorization-policy.md) | Accepted | Persist one global, binary Sending mode (Ask Before Sending / Send Automatically), app-owned and unreachable by any MCP caller, now wired into existing-conversation plain-text `messages_send` and picker-based `messages_send_attachment`. |
