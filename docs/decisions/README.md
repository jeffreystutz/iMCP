...
| [0009](0009-attachment-only-existing-chat-submission.md) | Superseded by [0011](0011-unified-messages-send-tool.md) (tool surface); [0014](0014-programmatic-attachment-source-ingress.md) (ingress) | Submit one picker-selected bounded file to one exact existing conversation through a separate attachment-only tool, with no path in MCP arguments and full post-confirmation revalidation. Its bounded-file/revalidation security contract is carried forward unchanged by ADR 0011/0014. |
...
| [0014](0014-programmatic-attachment-source-ingress.md) | Proposed | Replace `message_send_attachment`'s mandatory native picker with two programmatic sources: an allowed-folder `file_path`, or bounded serialized `filename`/`content_base64`. ADR 0009's bounded-file/revalidation/privacy contract carried forward unchanged; supersedes only ADR 0009's ingress mechanism. |
